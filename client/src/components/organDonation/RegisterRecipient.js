...
const RegisterRecipient = () => {
  ...
  const onSubmit = async (event) => {
    ...
    setRecipientData({ ...recipientData, loading: true, errMsg: '', successMsg: '' });

    const { fname, lname, gender, city, phone, email, bloodgroup, organ, buffer, publicKey } = recipientData;
    ...
    try {
      ...
      await blockchainData.OrganInstance.methods.addRecipient(publicKey, hospital.hospital.hospitalpublickey, recipientData.ipfsHash, recipientData.EMRHash, organ, bloodgroup).send({
        from: blockchainData.account,
      });
      setRecipientData({ ...recipientData, successMsg: "Repient Registered Successfully!" })
    }
    catch (err) {
    ...
    <Grid centered columns={2} style={{ marginTop: '20px' }}>
      <Grid.Column width={9}>
        <Segment>
          ...
          <Divider />
          <Form onSubmit={onSubmit} error={!!recipientData.errMsg} success={!!recipientData.successMsg}>
            <Form.Group widths={2}>
              <Form.Input
            ...
            <Message success header="Success" content={recipientData.successMsg} />
            <Segment basic textAlign={"center"}>
              <Button loading={recipientData.loading} positive type='submit'>Submit</Button>
            </Segment>
          </Form>
        </Segment>
      </Grid.Column>
